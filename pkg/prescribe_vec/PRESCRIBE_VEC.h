...
C------------------------------------------------------------------------------|
      ...
      INTEGER, PARAMETER :: nVEC_masks = 1
...
C     These are the file names for the depth level option
      CHARACTER*30 vec_depth_cell_fnames(N_VEC_FIELDS,nVEC_masks)
...
C     This is where the depth levels are stored when they are read in
C      - these may be provided by a file or a list
      _RL depth_mask(nVEC_masks,1-Olx:sNx+Olx,1-Oly:sNy+Oly,nSx,nSy)
...
C     This is where the local depth levels in the tile are stored
      INTEGER vec_depth_level(nVEC_masks, N_VEC_FIELDS, sNy*sNx)
